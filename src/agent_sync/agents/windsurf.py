...
class WindsurfAgent(BaseAgent):
    ...
    def get_source_paths(self) -> List[Path]:
        """Get all source paths for Windsurf skills."""
        paths = []
    ...
    def sync_skills(self, source: Optional[Path] = None, dry_run: bool = False) -> List[str]:
        """
        Sync skills FROM Windsurf directories TO ~/.agents/skills/.
    ...
    def sync_to_windsurf(self, source: Optional[Path] = None, dry_run: bool = False) -> List[str]:
        """
        Sync skills TO Windsurf FROM ~/.agents/skills/.
        ...
        windsurf_skills_dir = self._expand_path("~/.codeium/windsurf/skills/")
        if not dry_run:
            windsurf_skills_dir.mkdir(parents=True, exist_ok=True)
        ...
        for skill_dir in source.iterdir():
            if skill_dir.is_dir() and not skill_dir.name.startswith("."):
                skill_file = skill_dir / "SKILL.md"
                if skill_file.exists():
                    dest = windsurf_skills_dir / skill_dir.name
    ...
    def get_all_skills_paths(self) -> List[Path]:
        """Get all skills paths for Windsurf."""
