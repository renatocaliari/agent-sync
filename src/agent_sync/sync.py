            ...
            for src_path in paths:
                if src_path.exists():
                    ...
                    for item in src_path.iterdir():
                        ...
                        if item.is_dir():
                            shutil.copytree(item, item_dest, dirs_exist_ok=True, ignore=shutil.ignore_patterns('.git'), symlinks=True)
                        else:
        ...
        if hasattr(agent, 'packages_paths'):
            for package_path in agent.packages_paths:
                if package_path.exists():
                    ...
                    shutil.copytree(package_path, pkg_dest, ignore=shutil.ignore_patterns('.git'), symlinks=True)
            ...
            for package_path in agent.packages_paths:
                ...
                for package_item in synced_packages_dir.iterdir():
                    ...
                    if package_item.is_dir():
                        if dest.exists():
                            shutil.rmtree(dest)
                        shutil.copytree(package_item, dest, ignore=shutil.ignore_patterns('.git'), symlinks=True)
                        ...
                        for source_path_str in source_paths:
                            ...
                            for item in synced_category_dir.iterdir():
                                ...
                                if item.is_dir():
                                    if dest.exists():
                                        shutil.rmtree(dest)
                                    shutil.copytree(item, dest, ignore=shutil.ignore_patterns('.git'), symlinks=True)
                                else:
                                    shutil.copy2(item, dest, follow_symlinks=False)
