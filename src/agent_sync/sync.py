                ...
                for ext_path in agent.extensions_paths:
                    if ext_path.exists():
                        ...
                        for ext_item in ext_path.iterdir():
                            ...
                            if ext_item.is_dir():
                                shutil.copytree(ext_item, dest, symlinks=True, dirs_exist_ok=True)
                            else:
    ...
    def _copy_directory(
        ...
        for item in src.rglob("*"):
            ...
            if item.is_symlink():
                ...
                # Security: use follow_symlinks=False to prevent content leakage
            ...
            if sync_configs and synced_config_dir.exists() and agent.config_path.parent.exists():
                for config_file in synced_config_dir.glob("*"):
                    if config_file.is_file():
                        ...
                        if not dest.exists() or (not config_file.is_symlink() and dest.read_text() != config_file.read_text()):
                            shutil.copy2(config_file, dest, follow_symlinks=False)
                ...
                if synced_ext_dir.exists():
                    ...
                    for ext_path in agent.extensions_paths:
                        ...
                        for ext_item in synced_ext_dir.iterdir():
                            ...
                            if not dest.exists() or (ext_item.is_file() and not ext_item.is_symlink() and dest.read_text() != ext_item.read_text()):
                                if ext_item.is_dir():
                                    shutil.copytree(ext_item, dest, symlinks=True, dirs_exist_ok=True)
                                else:
                                    shutil.copy2(ext_item, dest, follow_symlinks=False)
                ...
                if synced_prompts_dir.exists():
                    ...
                    for prompts_path in agent.prompts_paths:
                        ...
                        for prompt_item in synced_prompts_dir.iterdir():
                            ...
                            if not dest.exists() or (prompt_item.is_file() and not prompt_item.is_symlink() and dest.read_text() != prompt_item.read_text()):
                                if prompt_item.is_dir():
                                    shutil.copytree(prompt_item, dest, symlinks=True, dirs_exist_ok=True)
                                else:
                ...
                if synced_themes_dir.exists():
                    ...
                    for themes_path in agent.themes_paths:
                        ...
                        for theme_item in synced_themes_dir.iterdir():
                            ...
                            if not dest.exists() or (theme_item.is_file() and not theme_item.is_symlink() and dest.read_text() != theme_item.read_text()):
                                if theme_item.is_dir():
                                    shutil.copytree(theme_item, dest, symlinks=True, dirs_exist_ok=True)
                                else:
            ...
            if project_agents_src.exists() and agent.agents_path:
                ...
                for agent_file in project_agents_src.rglob("*.md"):
                    if agent_file.is_file():
                        ...
                        if not dest.exists() or (not agent_file.is_symlink() and dest.read_text() != agent_file.read_text()):
                            shutil.copy2(agent_file, dest, follow_symlinks=False)
    ...
    def _apply_synced_skills(self) -> list[str]:
        ...
        if synced_skills_dir.exists():
            ...
            for skill_item in synced_skills_dir.glob("*"):
                ...
                if not dest.exists() or (skill_item.is_file() and not skill_item.is_symlink() and dest.read_text() != skill_item.read_text()):
                    if skill_item.is_dir():
                        shutil.copytree(skill_item, dest, symlinks=True, dirs_exist_ok=True)
                    else:
                        shutil.copy2(skill_item, dest, follow_symlinks=False)
    ...
    def _restore_extension_skills(self, manifest: dict) -> int:
        ...
        for ext_name, ext_info in manifest.get("extensions", {}).items():
            ...
            for skill_item in source_dir.iterdir():
                ...
                if skill_item.is_dir():
                    shutil.copytree(skill_item, dest_skill, symlinks=True, dirs_exist_ok=True)
                else:
