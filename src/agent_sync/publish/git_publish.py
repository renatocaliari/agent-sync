...
def do_git_publish(
    ...
    try:
        ...
        for src_path, dest_name in items:
            ...
            if src_path.is_dir():
                # symlinks=True: Preserve symlinks as symlinks instead of copying target content.
                # This prevents sensitive data leakage when publishing.
                shutil.copytree(src_path, dest, dirs_exist_ok=True, symlinks=True)
            else:
                dest.parent.mkdir(parents=True, exist_ok=True)
                # follow_symlinks=False: Do not follow symlinks; copy the link itself.
