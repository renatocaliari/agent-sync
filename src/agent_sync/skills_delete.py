...
class SkillsDeleter:
    ...
    def delete_skills(self, skill_names: List[str], dry_run: bool = False) -> dict:
        ...
        for skill_name in skill_names:
            # 1. Validate skill name to prevent path traversal
            if not validate_skill_name(skill_name):
                stats["errors"] += 1
                console.print(f"[red]✗ Invalid skill name: {skill_name}[/red]")
                continue

            # 2. Defense-in-depth: Resolve and verify path is within hub
            ...
            for agent in self.agents:
                ...
                # Defense-in-depth for agent directories
                agent_skill_path = (agent_skills_path / skill_name).resolve()
                try:
                    agent_skill_path.relative_to(agent_skills_path.resolve())
                except ValueError:
                    stats["errors"] += 1
                    console.print(f"[red]✗ Security error: Attempted path traversal in {agent.name}[/red]")
                    continue
