    ...
    def delete_skills(self, skill_names: List[str], dry_run: bool = False) -> dict:
        ...
        for skill_name in skill_names:
            ...
            try:
                ...
                # Defense in depth: Ensure the resolved path is inside the global skills directory
                hub_skill_path.relative_to(self.global_skills_dir.resolve())
            except (ValueError, RuntimeError):
                stats["errors"] += 1
                console.print(
                    f"[red]✗ Security: Path traversal attempt blocked: {skill_name}[/red]"
                )
                continue
            ...
            if not dry_run:
                try:
                    ...
                    console.print(
                        f"[green]✓ Deleted[/green] {skill_name} from hub ({hub_files} files)"
                    )
            ...
            for agent in self.agents:
                ...
                try:
                    ...
                    # Defense in depth: Ensure the resolved path is inside the agent skills directory
                    agent_skill_path.relative_to(agent_skills_path.resolve())
                except (ValueError, RuntimeError):
                    stats["errors"] += 1
                    console.print(
                        f"[red]✗ Security: Path traversal attempt blocked for {agent.name}: {skill_name}[/red]"
                    )
                    continue
                ...
                if agent_skill_path.exists():
                    ...
                    if not dry_run:
                        try:
                            ...
                            stats["errors"] += 1
                            console.print(
                                f"[red]✗ Error deleting {skill_name} from {agent.name}: {e}[/red]"
                            )
                    else:
                        console.print(
                            f"[dim]Would delete {skill_name} from {agent.name} ({agent_files} files)[/dim]"
                        )

            if not dry_run and agent_files_total > 0:
                console.print(
                    f"[dim]  └─ {agent_files_total} files removed from agent directories[/dim]"
                )
